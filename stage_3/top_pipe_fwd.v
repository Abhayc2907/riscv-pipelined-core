// 5-stage pipelined RV32I core — Stage 3
// Adds EX/MEM -> EX and MEM/WB -> EX forwarding to resolve data hazards
// WITHOUT stalling. Dependent instructions can now sit back-to-back.
//
// NOT yet handled (still deferred to later stages):
//   - Load-use hazard (forwarding alone can't fix this — the loaded
//     value isn't ready until MEM, one cycle too late for the very
//     next instruction's EX stage. Needs a 1-cycle stall — Stage 4.)
//   - Branch/control hazards (Stage 5) — PC still always increments by 4.
module top_pipe_fwd (
    input wire clk,
    input wire rst
);
    ...
    // =========================================================
    // EX stage — forwarding inserted here
    // =========================================================
    // Sources available to forward FROM: the instruction currently in
    // MEM (EX/MEM reg outputs) and the instruction currently in WB
    // (MEM/WB reg outputs). Both sets of signals are defined further
    // below, but Verilog wires don't care about declaration order.
    wire [31:0] alu_result_MEM;    // EX/MEM forwarding source (data)
    wire [4:0]  rd_MEM;
    wire        reg_write_MEM;
    ...
    // 3:1 mux per operand: 00 = ID/EX value, 10 = EX/MEM result, 01 = WB result
    ...
    // alu_src still picks immediate vs register — forwarding happens to
    // the *register* value first, then alu_src decides whether to use it.
    ...
    // NOTE: write_data uses alu_b_fwd (the forwarded rs2 value), not the
    // raw rs2_data_EX — a store's value-to-write needs forwarding just
    // as much as an ALU operand does (e.g. "add x4,... / sw x4,0(x0)").
    ex_mem_reg u_ex_mem (
        .clk(clk), .rst(rst),
